...
class Crystal(object):
    ...
    def zcar(self):
        """ Returns the z Cartesian coordinates of the basis atoms """
        return self._rcar[:, 2]
    ...
    def set_rcar(self, new_rcar):
        """ Sets the [x,y,z] Cartesian coordinates of the basis atoms """
        self._rcar = new_rcar
    ...
    def rrel(self):
        """ Returns the [x,y,z] relative coordinates of the basis atoms """
        return self._rrel

    def xrel(self):
        """ Returns the x relative coordinates of the basis atoms """
        return self._rrel[:, 0]

    def yrel(self):
        """ Returns the y relative coordinates of the basis atoms """
        return self._rrel[:, 1]

    def zrel(self):
        """ Returns the z relative coordinates of the basis atoms """
        return self._rrel[:, 2]

    def set_rrel(self, new_rrel):
        """ Sets the [x,y,z] relative coordinates of the basis atoms """
        self._rrel = new_rrel
        self.update_rcar_from_rrel()

    def set_lattec(self, new_lattvec):
        """ Sets the lattice vectors of the crystal """
        self._lattvec = new_lattvec

    def set_dict_types(self, new_dict_types):
        self._dict = new_dict_types
        ...
        """
            ...
            Definitions:
                ...
                Rmat = [[ex'*ex, ex'*ey, ex'*ez],
                        ...
                        [ez'*ex, ez'*ey, ez'*ez]]
        """
        self.set_lattec(np.dot(self._lattvec, Rmatrix.T))
        self.update_rcar_from_rrel()
        self.clean_coordinates()

    def delete_atom(self, idnull):
        """ Delete atoms in the crystal  """
        self.set_rcar(np.delete(self._rcar, idnull, 0))
    ...
    def clean_coordinates(self, ndecimals=12):
        ...
        self._lattvec = np.round(self._lattvec, ndecimals)
        self._rrel = np.round(self._rrel, ndecimals)
        self.update_rcar_from_rrel()
    ...
    def create_supercell(self, n1, n2, n3, name=''):
        ...
        for ii in range(self._natoms):
            if len(supercell._rcar) == 0:
                ...
                supercell._rcar = np.append(supercell._rcar,
                        sclattpos+self._rcar[ii, :], axis=0)
                supercell._itype = np.append(supercell._itype,
                        (np.ones(n1*n2*n3)*self._itype[ii]).astype('int'), axis=0)
    ...
    def create_new_orientation(self, v1, v2, v3, n1=20, n2=20, n3=20,
        ...
        """
            ...
            Inputs:
                ...
                n1, n2, n3    - defines the size of the supercell of base_crystal
                                that will be used to find the output crystal.
                                Default: n1=n2=n3=20
    ...
    def find_distance_peroidic(self):
        ...
        xrel_shift, yrel_shift, zrel_shift = np.meshgrid(\
                ...
                np.arange(0., 1.5, 0.5))
        rrel_shift = np.array([xrel_shift.flatten(), yrel_shift.flatten(), \
                zrel_shift.flatten()]).T

        self._ndist = np.ones([self.natoms(), self.natoms()]) * np.inf

        for ii in range(rrel_shift.shape[0]):
            sys.stdout.write(\
                    "\rGetting neighbor distances (periodic): %3d%%"\
                    %((ii+1.)/rrel_shift.shape[0] * 100))
            sys.stdout.flush()
            # shift the crystal
            self.translate_relative(rrel_shift[ii, :])
    ...
    def get_neighbor_list(self, dmax, dmin=0.):
        ...
        for ii in range(self.natoms()):
            idneighbors, = np.where((self.ndist()[:, ii] > dmin)&\
                                    (self.ndist()[:, ii] < dmax))
...
if __name__ == "__main__":
    ...
    rnorms = np.linalg.norm(sc.rcar(), axis=1)
    rnorms = np.repeat(rnorms, 3).reshape(sc.natoms(), 3)
